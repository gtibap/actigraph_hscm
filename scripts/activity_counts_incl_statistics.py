...
def statistics_motion(mag_col, inclinometer_pos):
    min_value = 3 # min intensity value
    vector_mag = mag_col['Vector Magnitude'].to_numpy()
    # vector_mag[vector_mag<=min_value] = 0
    # when 'Vector Magnitude' greater than 'min_value' and in the selected position ('Inclinometer Off' or 'Inclinometer Standing/Sitting')
    mask_pos = np.logical_not( (mag_col['Vector Magnitude'] > min_value) & (mag_col[inclinometer_pos]) )
    vector_mag_masked =  ma.array(vector_mag, mask=mask_pos)

    # print('len(vector_mag_masked): ', len(vector_mag_masked))
    # print ('len(vector_mag_masked.mask)', len(vector_mag_masked.mask), np.sum(vector_mag_masked.mask))

    # print('mean vector: ', vector_mag_masked.mean())
    # print('std vector: ', vector_mag_masked.std())
    # print('median vector: ', ma.median(vector_mag_masked))
    # print('min vector: ', vector_mag_masked.min())
    # print('max vector: ', vector_mag_masked.max())

    # plt.hist(vector_mag_masked)
    # plt.show()

    total_samples = len(vector_mag_masked.mask)
    valid_samples = len(vector_mag_masked.mask) - np.sum(vector_mag_masked.mask)

    # calculating statistics if there are valid values; if not make the statistics values equal to zero
    if valid_samples > 0:
        min=vector_mag_masked.min()
        max=vector_mag_masked.max()
        median=ma.median(vector_mag_masked)
        mean=vector_mag_masked.mean()
        std=vector_mag_masked.std()
    else:
        min=0
        max=0
        median=0
    ...
    inclinometer_samples = mag_col[inclinometer_pos].sum()
    return min,max,median,mean,std,valid_samples, inclinometer_samples, total_samples
...
if __name__== '__main__':
    ...
    path_out = "../data/results_vector_mag/"
    ...
    for sample in files_list:
        ...
        try:
            ...
            df1 = pd.read_csv(path+sample, header=header_location, decimal=',', usecols=['Date',' Time', inclinometer_pos, 'Vector Magnitude'])
            ...
            for night_num in nights_list:
                ...
                df_mag = df_nights.loc[(df_nights['night']==night_num), [inclinometer_pos, 'Vector Magnitude']]
                ...
                min,max,median,mean,std,valid_samples, inclinometer_samples, total_samples = statistics_motion(df_mag, inclinometer_pos)
                ...
                # df_night_stat = pd.DataFrame([night_num, min, max, median, mean, std,valid_samples,inclinometer_samples,total_samples],columns=['night','min','max','median','mean','std','valid_samples','inclinometer_samples','total_samples'])
                # night_stat = pd.Series({'night': night_num,'min':min_a,'max':max_a,'median':median_a,'mean':median_a,'std':std_a,'valid_samples':valid_samples_a,'inclinometer_samples':inclinometer_samples_a,'total_samples':total_samples_a})

                    # [night_num, min, max, median, mean, std,valid_samples,inclinometer_samples,total_samples],columns=['night','min','max','median','mean','std','valid_samples','inclinometer_samples','total_samples'])

                # print('night_stat: ', night_stat)
# 
                # df_statistics = pd.concat([df_statistics, pd.DataFrame(night_stat, columns=night_stat.index)]).reset_index(drop=True)
                # df_statistics.append(['night_num','min','max','median','mean','std','valid_samples','inclinometer_samples','total_samples'])

                # df_statistics.loc[night_num-1]=

                # chest        
                # duration of continuous periods of actitity and inactivity in seconds
                activity, inactivity, start_active = non_motion_periods(df_mag, inclinometer_pos)
