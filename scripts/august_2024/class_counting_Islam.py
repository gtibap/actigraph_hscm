...
class Counting_Actigraphy:
    ...
    def nightCounts(self):
        ...
        for date0, date1 in zip(dates_list[:-1], dates_list[1:]):
            ...
            ## counting number of repositioning 
            print(f'duration lying...')
            arr_length = self.counting_repositioning_duration(df_night)
    ...
    def counting_repositioning_duration(self, df):
        ...
        # ## from off to lying
        # ## spending time on lying before changing posture
        # ## first identify ids where lying starts
        # # print(f'arr_off_lyi: {arr_off_lyi}')
        # lyi_idx = np.argwhere(arr_off_lyi==True).flatten()
        # # print(f'lyi_idx: {lyi_idx}')
        # ## find index where current lying finishes
        # arr_acc = arr_lyi_off | arr_lyi_sit | arr_lyi_sta
        # arr_idx = np.argwhere(arr_acc==True).flatten()
        # # print(f'arr_idx: {arr_idx}')
        # ## for each lying segment find its respective end
        # lyi_length = list()
        ...
        #     ## minimum length lying segment
        #     if len(idx) > 0:
        #         lyi_length.append(idx[0] - id)
        #     else:
        #         pass
        
        # print(f'lyi length: {lyi_length}')

        ##### off lying
        arr_acc = arr_lyi_off | arr_lyi_sit | arr_lyi_sta
        l_length = self.length_segments_method(arr_off_lyi, arr_acc)
        print(f'lyi-off: {l_length}')

        ##### lying off
        arr_acc = arr_off_lyi | arr_off_sit | arr_off_sta
        l_length = self.length_segments_method(arr_lyi_off, arr_acc)
        print(f'off-lyi: {l_length}')
