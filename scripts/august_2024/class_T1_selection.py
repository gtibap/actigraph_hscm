...
class Counting_Actigraphy:
    ...
    def getAverages(self):
        ...
        list_days = ['d2','d3','d4','d5','d6']
        list_nights = ['n2','n3','n4','n5','n6']
        ...
        labels_list = df[self.label_day_night].unique().tolist()
        # print(f'labels_list: {labels_list}')
        arr_days = np.array([])
        arr_nights = np.array([])

        for label in labels_list:
            df_period = df[df[self.label_day_night]== label]
            arr_ar = df_period[self.vma_b].to_list()

            # print (f'label: {label}')    
            # if label.startswith('d'):
            if label in list_days:
                arr_days=np.concatenate((arr_days, arr_ar), axis=None)
            elif label in list_nights:
                arr_nights=np.concatenate((arr_nights, arr_ar), axis=None)
            else:
                pass

        arr_all = np.concatenate((arr_days,arr_nights),axis=None)

        # print(f'mean, median: {np.mean(arr_all)}, {np.median(arr_all)}')

        # arr_b = self.df1[self.vma_b]
        # ax[0].hist(arr_days, bins=50)
        # ax[1].hist(arr_nights, bins=50)
        # ax[-1].set_xlim([0,1])

        return np.mean(arr_all), np.median(arr_all), np.mean(arr_days), np.std(arr_days), np.mean(arr_nights), np.std(arr_nights) 
