...
def main(args):
    ...
    files_list_chest = [ name+'_chest.csv' for name in files_names]
    # prefix_two_chest='chest_'
    # body_part_title_chest = 'Chest'
    # elif body_part == 't':
    
    files_list_thigh = [ name+'_thigh.csv' for name in files_names]
    # prefix_two_chest='thigh_'
    # body_part_title_thigh = 'Thigh'
    # else:
    #     print(f'selected one body part out of the list=[c,t], being c for chest, or t for thigh.')
    #     return 0
        
    list_objs_chest = [[]]*len(files_list_chest)
    list_objs_thigh = [[]]*len(files_list_thigh)
    
    for i,filename in enumerate(files_list_chest):
        try:
            print(f'reading file: {filename}... ', end='')
            # create object class
            list_objs_chest[i] = Activity_Measurements(files_names[i])
            list_objs_chest[i].openFile(path, filename)
            print(f'done.')
            
        except ValueError:
            print(f'Problem reading the file {filename}.')

    for i,filename in enumerate(files_list_thigh):
        try:
            print(f'reading file: {filename}... ', end='')
            # create object class
            list_objs_thigh[i] = Activity_Measurements(files_names[i])
            list_objs_thigh[i].openFile(path, filename)
            print(f'done.')
    ...
    if flag_continue:
        ...
        ## chest ##
        for i, obj in enumerate(list_objs_chest):
        ...
        ## thigh ##
        for i, obj in enumerate(list_objs_thigh):
